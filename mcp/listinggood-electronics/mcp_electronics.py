#!/usr/bin/env python3
"""ListingGood · Electronics Compliance 垂直 MCP（远程托管版 · streamable-http）。

首个垂直品类 MCP（来自付费转化策略 v2.0 §4）：电子/电器（CE/EMC/RoHS/WEEE/GPSR/EPR/FCC）。
定位 = **免费诊断引流**，把"怕合规/怕下架"的卖家在离痛点最近的地方（AI agent 工作流）捞进来，
...
- 本 MCP 只做诊断（免费），不收费；收费动作一律指向站点 /compliant（per-SKU 文件包，5 星/SKU）。

运行（服务器，独立 venv / 独立端口 8051，零侵入主站）：
    LISTINGGOOD_API_BASE=http://127.0.0.1:8020 \
    python mcp_electronics.py            # 监听 127.0.0.1:8051，nginx 反代 /mcp-electronics

计费：本 MCP 全部工具免费（诊断引流）；付费在站点侧。
依赖：mcp>=1.3（streamable_http_app）、uvicorn、starlette（mcp 自带）。
工具清单（3 个，均免费、无需 Key）：
- electronics_market_requirements → 返回目标市场电子产品的必备标志/法规框架（内置知识）
- electronics_compliance_scan      → /api/free-check 风险扫描 + 电子专项缺口叙述（免费，无需 Key）
- electronics_file_package         → 返回站点 /compliant 付费文件包链接与说明（导流）
"""
import os
import json
...
# 电子产品（按市场区域）必备标志/法规框架 —— 内置知识，独立于主站模块。
FRAMEWORKS = {
    "EU": [
        ("CE marking", "进入欧盟市场的强制合格标志"),
        ("EMC Directive 2014/30/EU", "电磁兼容"),
        ("Low Voltage Directive (LVD) 2014/35/EU", "电气设备安全"),
        ("RoHS 3 (2015/863)", "限制铅/镉/汞等 10 项有害物质"),
        ("WEEE 2012/19/EU", "废弃电子电气设备，需注册回收"),
        ("GPSR (EU 2023/988)", "通用产品安全法规：欧盟负责人(EC REP)与可追溯性"),
        ("REACH (EC 1907/2006)", "化学品注册/通报（含 SVHC）"),
        ("EcoDesign / ERP", "能效与生态设计"),
        ("Battery Regulation (EU 2023/1542)", "含电池产品的电池合规"),
    ],
    "US": [
        ("FCC Part 15 (EMC / RF)", "电磁兼容与射频设备认证（SDoC 或 Certification）"),
        ("UL / ANSI safety (平台常要求)", "电气安全，零售/平台通常要求"),
        ("California Prop 65", "含已知致癌/生殖毒性物质需警示"),
        ("CPSC 16 CFR", "一般消费品安全（含溯源标签要求）"),
    ],
    "UK": [
        ("UKCA marking", "英国合格评定标志（过渡期与 CE 并行）"),
        ("UK REACH", "英国化学品法规"),
        ("WEEE (UK)", "英国废弃电子电气设备"),
    ],
}
...
_OUTPUT_SCHEMAS = {
    "electronics_market_requirements": _TOOL_OUTPUT_SCHEMA,
    "electronics_compliance_scan": _TOOL_OUTPUT_SCHEMA,
    "electronics_file_package": _TOOL_OUTPUT_SCHEMA,
}
...
mcp = FastMCP("listinggood-electronics", streamable_http_path="/mcp-electronics")
...
def electronics_market_requirements(marketplace: str = "EU", lang: str = "en") -> str:
    """返回目标市场电子/电器产品的必备标志与法规框架清单（内置知识，免费、无需 Key）。
    ...
    if lang == "zh":
        lines = ["# 电子产品 · %s 市场必备标志 / 法规框架" % region,
                 "> 来源：ListingGood 电子产品合规知识库（诊断用，非认证结论）。\n"]
        for n, d in fw:
            lines.append("- **%s** — %s" % (n, d))
        lines.append("\n下一步：在 listinggood.com/compliant 生成单 SKU 的「合规证据文件包」"
                     "（符合性声明 DoC + 风险评估 + 标志清单）。")
    else:
        lines = ["# Electronics · required marks / frameworks for %s" % region,
                 "> Source: ListingGood electronics compliance knowledge (diagnostic, not a certification).\n"]
        for n, d in fw:
            lines.append("- **%s** — %s" % (n, d))
...
@mcp.tool(annotations=ToolAnnotations(read_only_hint=True, destructive_hint=False, idempotent_hint=False, open_world_hint=True))
def electronics_compliance_scan(text: str, marketplace: str = "US", lang: str = "en") -> str:
    """免费电子合规风险扫描（无需 Key）：对粘贴的 Listing 文案做确定性规则体检，返回风险点 +
    已通过项，并附电子专项缺口叙述。完整「合规证据文件包」（DoC/风险评估/标志清单）在站点付费生成。
    text: 标题+五点+描述原文；marketplace: 站点代码；lang: zh/en。"""
    payload = {"text": text, "marketplace": marketplace, "lang": lang, "source": "mcp-electronics"}
    ...
    if lang == "zh":
        note = ("\n\n## 电子专项提示\n该市场电子产品通常还需具备：%s。"
                "上述为「诊断」，完整的符合性声明(DoC)、风险评估与标志清单请到 "
                "https://listinggood.com/compliant 生成（按 SKU 收费）。"
                % "、".join(n for n, _ in fw))
    else:
        note = ("\n\n## Electronics note\nProducts in this market typically also need: %s. "
                "The above is a diagnosis only. Generate the full Declaration of Conformity, "
                "risk assessment and marks checklist at https://listinggood.com/compliant (per-SKU)."
                % ", ".join(n for n, _ in fw))
    body["electronics_note"] = note
    return json.dumps(body, ensure_ascii=False, indent=2)


@mcp.tool(annotations=ToolAnnotations(read_only_hint=True, destructive_hint=False, idempotent_hint=False, open_world_hint=True))
def electronics_file_package(sku: str, marketplace: str = "US", lang: str = "en") -> str:
    ...
    if lang == "zh":
        ...
        out = ("# Compliance evidence file package (per-SKU, paid)\n"
               ...
               "- The blue \"Compliance file ready\" mark is a free visual byproduct once files are assembled; "
               "ListingGood issues no certification verdict.\n"
               "- Cost: 5 compliance checks per SKU (≈ $0.50), deducted from account balance." % (url, sku, marketplace))
...
def build_app():
    ...
    # 健康检查必须放在 /mcp-electronics 前缀之外，否则会被 FastMCP session 中间件
    # 拦截并强制要求 Accept: text/event-stream，导致监控拿到 JSON-RPC 错误而非 200。
    app.add_route("/lg-electronics-health",
                  lambda r: JSONResponse({"ok": True, "service": "listinggood-mcp-electronics"}))
    return app
