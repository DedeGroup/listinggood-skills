#!/usr/bin/env python3
"""ListingGood · ListingGood Furniture Compliance MCP（远程托管版 · streamable-http）。

垂直品类 MCP（来自付费转化策略 v2.0 §4）：家具（Furniture）。定位 = **免费诊断引流**，
...
运行（服务器，独立 venv / 独立端口 8054，零侵入主站）：
    LISTINGGOOD_API_BASE=http://127.0.0.1:8020 \
    python mcp_furniture.py            # 监听 127.0.0.1:8054，nginx 反代 /mcp-furniture

计费：本 MCP 全部工具免费（诊断引流）；付费在站点侧。
依赖：mcp>=1.3（streamable_http_app）、uvicorn、starlette（mcp 自带）。
工具清单（3 个，均免费、无需 Key）：
- furniture_market_requirements → 返回目标市场家具产品的必备标志/法规框架（内置知识）
- furniture_compliance_scan      → /api/free-check 风险扫描 + 家具专项缺口叙述（免费，无需 Key）
- furniture_file_package         → 返回站点 /compliant 付费文件包链接与说明（导流）
"""
import os
import json
...
PORT = int(os.environ.get("MCP_PORT", "8054"))

API_KEY_CTX: ContextVar = ContextVar("api_key", default=None)

# 家具（按市场区域）必备标志/法规框架 —— 内置知识，独立于主站模块。
FRAMEWORKS = {
    'EU': [
            ('EN 1021 (家具阻燃 - 香烟/火柴)', '软体家具阻燃（阴燃 / 明火）'),
            ('EN 12520 / EN 1730 (强度/耐久/稳定)', '座椅强度耐久与稳定性'),
            ('REACH (EC 1907/2006)', '甲醛 / 阻燃剂 / SVHC 限制'),
            ('GPSR (EU 2023/988)', '通用产品安全法规（责任人 + 可追溯）'),
            ('EN 716 (儿童床/童椅)', '儿童家具安全'),
            ('软垫家具成分与阻燃标签 (部分成员国)', '复合材质成分与阻燃标签'),
    ],
    'US': [
            ('16 CFR 1632 (床垫阻燃 - 香烟)', '床垫阴燃阻燃'),
            ('16 CFR 1633 (床垫阻燃 - 明火)', '床垫明火阻燃'),
            ('CAL TB 117-2013 (加州软体家具阻燃)', '加州软体家具阻燃'),
            ('ASTM F2057 (储物柜防倾倒)', '衣物收纳柜防倾倒'),
            ('CPSIA (儿童家具 铅/邻苯)', '儿童家具铅与邻苯限制'),
            ('California Prop 65', '含甲醛/阻燃剂/铅需警示'),
            ('TB 133 (公共座椅阻燃)', '公共场所座椅阻燃'),
            ('ANSI/BIFMA (商用)', '商用家具标准'),
    ],
    'UK': [
            ('UK Furniture and Furnishings (Fire Safety) Regulations 1988', '英国家具阻燃（填充物 + 面料 + 标签）'),
            ('UK REACH', '英国化学品法规'),
            ('UKCA', '英国合格评定标志'),
            ('GPSR (UK)', '通用产品安全法规'),
    ],
...
_OUTPUT_SCHEMAS = {
    "furniture_market_requirements": _TOOL_OUTPUT_SCHEMA,
    "furniture_compliance_scan": _TOOL_OUTPUT_SCHEMA,
    "furniture_file_package": _TOOL_OUTPUT_SCHEMA,
}
...
mcp = FastMCP("listinggood-furniture", streamable_http_path="/mcp-furniture")
...
def furniture_market_requirements(marketplace: str = "EU", lang: str = "en") -> str:
    """返回目标市场家具产品的必备标志与法规框架清单（内置知识，免费、无需 Key）。
    ...
    if lang == "zh":
        lines = ["# 家具 · %s 市场必备标志 / 法规框架" % region,
                 "> 来源：ListingGood 家具合规知识库（诊断用，非认证结论）。\n"]
        for n, d in fw:
            lines.append("- **%s** — %s" % (n, d))
        lines.append("\n下一步：在 listinggood.com/compliant 生成单 SKU 的「合规证据文件包」"
                     "（符合性声明 DoC + 风险评估 + 标志清单）。")
    else:
        lines = ["# Furniture · required marks / frameworks for %s" % region,
                 "> Source: ListingGood Furniture compliance knowledge (diagnostic, not a certification).\n"]
        for n, d in fw:
            lines.append("- **%s** — %s" % (n, d))
...
@mcp.tool(annotations=ToolAnnotations(read_only_hint=True, destructive_hint=False, idempotent_hint=False, open_world_hint=True))
def furniture_compliance_scan(text: str, marketplace: str = "US", lang: str = "en") -> str:
    """免费家具合规风险扫描（无需 Key）：对粘贴的 Listing 文案做确定性规则体检，返回风险点 +
    已通过项，并附家具专项缺口叙述。完整「合规证据文件包」（DoC/风险评估/标志清单）在站点付费生成。
    text: 标题+五点+描述原文；marketplace: 站点代码；lang: zh/en。"""
    payload = {"text": text, "marketplace": marketplace, "lang": lang, "source": "furniture"}
    ...
    fw = FRAMEWORKS.get(region, [])
    if lang == "zh":
        note = ('\n\n## 家具专项提示\n该市场家具产品通常还需具备：%s。上述为「诊断」，完整的阻燃/化学物质合规清单与测试文件清单请到 https://listinggood.com/compliant 生成（按 SKU 收费）。' % "、".join(n for n, _ in fw))
    else:
        note = ('\n\n## Furniture note\nFurniture in this market typically also needs: %s. The above is a diagnosis only. Generate the full flammability/chemical compliance checklist and test-file list at https://listinggood.com/compliant (per-SKU).' % ", ".join(n for n, _ in fw))
    body["note"] = note
    return json.dumps(body, ensure_ascii=False, indent=2)


@mcp.tool(annotations=ToolAnnotations(read_only_hint=True, destructive_hint=False, idempotent_hint=False, open_world_hint=True))
def furniture_file_package(sku: str, marketplace: str = "US", lang: str = "en") -> str:
...
def build_app():
    ...
    # 健康检查必须放在 /mcp-furniture 前缀之外，否则会被 FastMCP session 中间件
    # 拦截并强制要求 Accept: text/event-stream，导致监控拿到 JSON-RPC 错误而非 200。
    app.add_route("/lg-furniture-health",
                  lambda r: JSONResponse({"ok": True, "service": "listinggood-mcp-furniture"}))
    return app
